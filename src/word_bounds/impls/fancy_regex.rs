...
const REGEX_CRATE_PATTERN: &str = r"_|-|(?<=[a-z0-9])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])";

#[cfg(not(feature = "optimize_for_memory"))]
static REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(REGEX_CRATE_PATTERN).expect("Expected valid fancy_regex pattern"));
...
impl WordBoundResolverLike for WordBoundResolverFancyRegex {
    fn resolve(s: &str) -> Vec<String> {
        ...
        let re = Regex::new(REGEX_CRATE_PATTERN).expect("Expected valid fancy_regex pattern");
