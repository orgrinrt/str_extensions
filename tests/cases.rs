...
fn every_spelling_converts_to_snake_case() {
    for input in EQUIVALENT_SPELLINGS {
        assert_eq!(input.to_snake_case(), "this_is_an_example", "input: {input}");
    }
}
...
fn every_spelling_converts_to_kebab_case() {
    for input in EQUIVALENT_SPELLINGS {
        assert_eq!(input.to_kebab_case(), "this-is-an-example", "input: {input}");
    }
}
...
fn every_spelling_converts_to_human_readable() {
    for input in EQUIVALENT_SPELLINGS {
        assert_eq!(input.to_human_readable(), "this is an example", "input: {input}");
    }
}
...
fn every_spelling_converts_to_title_case() {
    for input in EQUIVALENT_SPELLINGS {
        assert_eq!(input.to_title_case(), "This Is An Example", "input: {input}");
    }
}
...
fn acronyms_stay_whole() {
    ...
    assert_eq!("thisExampleHasIDELikeACRONYMS".to_snake_case(), "this_example_has_ide_like_acronyms");
}

/// Digits bound a word of their own.
#[test]
fn numbers_are_their_own_word() {
    assert_eq!("WordWithNumbers123".to_snake_case(), "word_with_numbers_123");
...
fn leading_separators_do_not_double_up() {
    assert_eq!("_PrependedUnderscore".to_snake_case(), "prepended_underscore");
